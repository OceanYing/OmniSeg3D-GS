...
def feature_to_rgb(features):
    ...
    sam_norm = features_reshaped / (features_reshaped.norm(dim=1, keepdim=True) + 1e-6)
    features_reshaped = sam_norm * 0.5 + 0.5
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        np.save(os.path.join(colormask_path, 'feature_{0:05d}'.format(idx) + ".npy"), rendering_obj.cpu().numpy())
