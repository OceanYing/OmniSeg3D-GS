...
class GaussianModel:
    ...
    def training_setup(self, training_args, sem_only_flag=False):
        ...
        # ]
        self.sem_only_flag = sem_only_flag
        if sem_only_flag:
            l = [
                {'params': [self._objects_dc], 'lr': training_args.omni_lr, "name": "obj_dc"},
            ]
        else:
            l = [
                {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
    ...
    def save_ply_partial(self, path, idx):
        ...
        obj_dc = self._objects_dc[idx, :].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
