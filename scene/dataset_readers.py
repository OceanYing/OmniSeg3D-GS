...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, objects_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        elif os.path.exists(image_path[:-4] + '.png'): 
            image = Image.open(image_path[:-4] + '.png') 
        else:
            print("image not exits: ", image_path)
            image = None
        
        if os.path.isfile(objects_folder):  # load original mask data
        ...
        else:   # load omniseg3d data
            object_acc_path = os.path.join(objects_folder, image_name + '.npz')
            sam = np.load(object_acc_path)
            objects = cv2.resize(sam['indices'], (image.size[0], image.size[1]), interpolation=cv2.INTER_NEAREST)
            # import pdb
            # pdb.set_trace()
            
            objects_accpatch = sam['correlation']

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, objects=objects, objects_accpatch=objects_accpatch)
        ...
        for idx, frame in enumerate(frames):
            ...
            if "train" in transformsfile:
                if os.path.isfile(objects_folder):  # load original mask data
                    ...
                    object_acc_path = os.path.join(path, objects_folder, image_name + '.npz')
                    sam = np.load(object_acc_path)
                    objects = sam['indices']

                    objects[im_data[..., -1] == 0] = 0      # skip supervise backgroud
                    objects_accpatch = sam['correlation']

            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                            image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], objects=objects, objects_accpatch=objects_accpatch))
